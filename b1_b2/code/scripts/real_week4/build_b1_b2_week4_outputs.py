...
def build_top30(feature_matrix: pd.DataFrame) -> pd.DataFrame:
    ...
    preferred = agg[agg["parameter_name"].isin(PREFERRED_CORE_PARAMS)].copy()
    selected = preferred.copy()
    remaining = agg[~agg["parameter_name"].isin(set(selected["parameter_name"]))].copy()
    selected = pd.concat([selected, remaining.head(max(0, 30 - len(selected)))], ignore_index=True)
    selected = selected.sort_values(
        ["importance_score", "safety_score", "raw_keyword_score"],
        ascending=[False, False, False],
    ).head(30)
    selected["rank"] = range(1, len(selected) + 1)
    selected["selection_reason"] = selected["parameter_name"].apply(
        lambda name: "week2_top5_core_prior" if name in PREFERRED_CORE_PARAMS else "offline_importance_rank"
    )
    return selected[
        [
            "rank",
...
def sync_to_b1_b2_module() -> None:
    ...
    for src in files:
        if src.suffix == ".md":
            dst = B1_B2 / "docs" / "week4" / src.name
        elif "action_space" in src.name:
            dst = B1_B2 / "interfaces" / "week4" / src.name
        else:
            dst = B1_B2 / "results" / "week4" / src.name
...
def main() -> int:
    ...
    print(json.dumps({
        ...
        "output_dir": str(OUT),
        "b1_b2_dir": str(B1_B2) if B1_B2.exists() else "",
