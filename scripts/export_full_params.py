#!/usr/bin/env python3
"""
OceanBase v4.5 关键性能参数完整导出（含源码机制说明）

合并4个维度的参数定义和源码机制分析，导出完整Excel
"""
...
def load_mechanisms():
    """加载4个维度的源码机制分析"""
    mechanism_map = {}
    for dim_file in ["cpu_params_mechanism.json", "memory_params_mechanism.json",
                     "disk_io_params_mechanism.json", "sql_params_mechanism.json"]:
        fpath = os.path.join(OUTPUT_DIR, dim_file)
        if os.path.exists(fpath):
            with open(fpath, 'r', encoding='utf-8') as f:
                data = json.load(f)
                for param_name, info in data.items():
                    mechanism_map[param_name] = {
...
def export_to_excel(params, param_map, mechanism_map, output_path):
    """导出完整参数文档到Excel"""
    wb = Workbook()
    ...
    dim_fills = {
        "CPU调度": PatternFill(start_color="DAEEF3", end_color="DAEEF3", fill_type="solid"),
        "内存管理": PatternFill(start_color="E2EFDA", end_color="E2EFDA", fill_type="solid"),
        "磁盘IO": PatternFill(start_color="FDE9D9", end_color="FDE9D9", fill_type="solid"),
        "SQL执行": PatternFill(start_color="F2DCDB", end_color="F2DCDB", fill_type="solid"),
    }
    ...
    # 表头
    headers = ["序号", "维度", "参数名", "当前值", "默认值", "取值范围", "数据类型", "编辑级别", "核心作用", "源码影响机制", "关键源码文件"]
    col_widths = [5, 8, 38, 14, 14, 28, 9, 16, 50, 70, 40]
    ...
    for idx, (name, dim, default_val, value_range, core_func) in enumerate(params, 1):
        ...
        mech_info = mechanism_map.get(name, {})
        ...
        mechanism = mech_info.get('mechanism', '暂无源码分析')
        code_files = '\n'.join(mech_info.get('code_files', []))

        row_data = [idx, dim, name, current_val, db_default, value_range,
                    data_type, edit_level, core_func, mechanism, code_files]
    ...
    # 行高：含机制的行需要更高
    for row in range(2, len(params) + 2):
        ws.row_dimensions[row].height = 90

    ws.freeze_panes = "A2"
    ws.auto_filter.ref = f"A1:K{len(params)+1}"

    # ---- Sheet 2: 维度汇总 ----
    ws2 = wb.create_sheet("维度汇总")
    summaries = [
        ("CPU调度", 25, "🔵 蓝色底色", "线程池、并发控制、Worker调度、网络IO线程"),
        ("内存管理", 25, "🟢 绿色底色", "内存配额、MemStore、缓存淘汰、SQL工作区"),
        ("磁盘IO", 25, "🟠 橙色底色", "数据文件、日志磁盘、IO调度、写入限流"),
        ("SQL执行", 25, "🔴 红色底色", "SQL审计、计划缓存、连接算法、并行查询"),
    ]
    s2_headers = ["维度", "参数数量", "颜色标识", "覆盖范围"]
    ...
    for row_idx, (dim, count, color, scope) in enumerate(summaries, 2):
        for col_idx, value in enumerate([dim, count, color, scope], 1):
            cell = ws2.cell(row=row_idx, column=col_idx, value=value)
            cell.font = cell_font
            cell.border = thin_border
            cell.alignment = center_alignment
        ws2.cell(row=row_idx, column=1).fill = dim_fills.get(dim)
...
def main():
    output_path = os.path.join(OUTPUT_DIR, "OceanBase_v4.5_关键性能参数_完整版.xlsx")

    print("=" * 60)
    print("OceanBase v4.5 关键性能参数完整导出（含源码机制）")
    print("=" * 60)

    # 1. 加载源码机制
    print("\n[1/3] 加载源码机制分析...")
    mechanism_map = load_mechanisms()
    covered = sum(1 for n, _, _, _, _ in KEY_PARAMS if n in mechanism_map)
    print(f"  ✓ 已加载 {len(mechanism_map)} 个参数的源码机制")
    print(f"  ✓ 覆盖率: {covered}/100 = {covered}%")
    ...
    # 3. 导出Excel
    print("\n[3/3] 导出完整Excel...")
    export_to_excel(KEY_PARAMS, param_map, mechanism_map, output_path)
    ...
    for dim, count in dims.items():
        m_count = sum(1 for n, d, _, _, _ in KEY_PARAMS if d == dim and n in mechanism_map)
        print(f"  {dim}: {count} 个参数 (源码机制覆盖 {m_count}/{count})")
    print(f"\n文件位置: {output_path}")
